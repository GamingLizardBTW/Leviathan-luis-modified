...
class singledriveWithJoystickCommand(commands2.Command):
    def __init__(self, drivetrainSubsystem: drivetrainSubsystemClass) -> None :
        ...
        self.controller = wpilib.XboxController(OP.single_controller)
        self.addRequirements(drivetrainSubsystem)
        logger.info("singledriveWithJoystick constructor")
    
    def initialize(self):
        logger.info("singledriveWithJoystick initiate")

    def execute(self):
        #forward is positive X and left is positive Y.
        self.leftY = XboxController(OP.single_controller).getLeftY() #This will be xSpeed(front and back)
        xSpeed = (-self.xSpeedLimiter.calculate(wpimath.applyDeadband(self.leftY, 0.08)) * OP.max_speed)
        #xSpeed = -self.leftY * OP.max_speed

        self.leftX = XboxController(OP.single_controller).getLeftX() #This will be ySpeed(left and right)
        ...
        if self.controller.getRawButton(4):
            ...
            self.rightX = XboxController(OP.single_controller).getRightX()#This will be rotation(turn heading left and right)
